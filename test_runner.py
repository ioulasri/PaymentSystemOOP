...
class TestResultParser:
    ...
    def parse_output(self, output: str):
        ...
        test_result_pattern = r'^(.*?)::(.*?)::(.*?)\s+(PASSED|FAILED|SKIPPED|ERROR)\s*(\[.*?\])?'
    ...
    def _parse_summary(self, lines):
        ...
        for line in lines:
            ...
            if 'passed' in line and ('failed' in line or 'error' in line or 'skipped' in line or 'warnings' in line):
                ...
                self.summary_info = {
                    'passed': int(passed_match.group(1)) if passed_match else len(self.passed_tests),
                    'failed': int(failed_match.group(1)) if failed_match else len(self.failed_tests),
                    'skipped': int(skipped_match.group(1)) if skipped_match else len(self.skipped_tests),
                    'errors': int(error_match.group(1)) if error_match else len(self.error_tests),
                    'warnings': int(warning_match.group(1)) if warning_match else len(self.warnings),
                    'total': 0
                }
                break
        ...
        if self.summary_info:
            self.summary_info['total'] = (self.summary_info['passed'] + 
...
class TestRunner:
    ...
    def _save_test_list(self, test_list, filename, header):
        ...
        if not test_list:
            ...
            for file_name, tests in sorted(by_file.items()):
                ...
                content += "-" * 40 + "\n"
                for test in tests:
                    status_emoji = "✅" if test['status'] == 'PASSED' else "❌" if test['status'] == 'FAILED' else "⏭️" if test['status'] == 'SKIPPED' else "💥"
    ...
    def _generate_summary(self, timestamp):
        ...
        if self.parser.summary_info:
            ...
            summary += f"│ Total Tests:     {total:<8} │ Pass Rate: {pass_rate:>6.1f}%     │\n"
            summary += f"│ ✅ Passed:       {passed:<8} │ ❌ Failed:      {failed:<8} │\n"
            summary += f"│ ⏭️ Skipped:      {skipped:<8} │ 💥 Errors:      {errors:<8} │\n"
        ...
        all_files = set()
        for test_list in [self.parser.passed_tests, self.parser.failed_tests, 
                         self.parser.skipped_tests, self.parser.error_tests]:
            for test in test_list:
                all_files.add(test['file'].split('/')[-1])
        
        if all_files:
            for file_name in sorted(all_files):
                file_passed = len([t for t in self.parser.passed_tests if t['file'].endswith(file_name)])
                file_failed = len([t for t in self.parser.failed_tests if t['file'].endswith(file_name)])
                file_skipped = len([t for t in self.parser.skipped_tests if t['file'].endswith(file_name)])
                file_errors = len([t for t in self.parser.error_tests if t['file'].endswith(file_name)])
